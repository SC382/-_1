  ...
  () => {
    ...
    const validateWorktabs = (routerInstance: Router): void => {
      try {
        ...
        /** 旧的演示「首页」(/home) 不应再出现在工作台标签（路由已移除） */
        const isLegacyHomeWorktab = (tab: Partial<WorkTab>): boolean => tab.path === "/home";

        /** 已合并/删除的菜单（医院人员管理已并入医院管理），避免工作台残留路由指向 404 */
        const isRemovedMenuWorktab = (tab: Partial<WorkTab>): boolean =>
          tab.path === "/cpx/hospital-user";

        // 过滤出有效的标签页
        const validTabs = opened.value.filter(
          (tab) => isTabRouteValid(tab) && !isLoginWorktab(tab) && !isLegacyHomeWorktab(tab) && !isRemovedMenuWorktab(tab)
        );
