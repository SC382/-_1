...
import { setBizMode } from "@/utils/biz-mode";
import AuthAPI, { type LoginFormData } from "@/api/module_system/auth";
import UserAPI from "@/api/module_system/user";
import CpxAuthAPI, { type BizUserInfo, type LoginParams } from "@/api/cpx/auth";
...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    // 是否业务登录（user_account，胸痛中心 Web 端）
    const isBusinessLogin = ref(false);
    // 业务用户信息（胸痛中心）
    const businessInfo = ref<BizUserInfo | null>(null);
    // 锁屏状态
    ...
    /**
     * 业务登录（胸痛中心 Web 端，基于 user_account）
     * 成功后写入 token 与业务用户信息，供守卫构建静态角色菜单。
     */
    async function businessLogin(loginForm: LoginParams): Promise<BizUserInfo> {
      const response = await CpxAuthAPI.login(loginForm);
      const data = response.data.data;
      if (response.data.code === ResultEnum.SUCCESS) {
        ElNotification({
          title: "通知",
      ...
      resetRouteInitState();
      Auth.setTokens(accessToken, "", rememberMe.value);
      ...
      Object.assign(info.value, {
        ...
        real_name: data.userinfo.real_name,
        // 供 MenuProcessor.processFrontendMenu 按 role_code 过滤内置菜单
        roles: [{ code: data.userinfo.role_code }],
      });
      setLoginStatus(true);
    ...
    /**
     * 恢复业务会话（F5 刷新场景）：优先请求业务 /userinfo 判定业务登录态。
     * 成功返回 true；否则返回 false（交由框架流程处理）。
     */
    async function tryRestoreBusinessSession(): Promise<boolean> {
      if (!Auth.getAccessToken()) return false;
      try {
        const response = await CpxAuthAPI.userinfo();
        const data = response.data.data;
        ...
        Object.assign(info.value, {
          ...
          roles: [{ code: data.role_code }],
        });
        setLoginStatus(true);
        return true;
      } catch (error) {
        // token 失效（401/403）：同步清除本地认证状态，避免守卫在登录页/首页之间死循环
    ...
    async function businessLogout() {
      ...
      if (token) {
        try {
          await CpxAuthAPI.logout();
        } catch {
          // 忽略登出接口异常
        }
      }
      businessInfo.value = null;
      isBusinessLogin.value = false;
      setBizMode(false);
      resetAllState();
