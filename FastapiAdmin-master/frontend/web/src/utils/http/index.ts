...
import { isBizMode } from "@/utils/biz-mode";
...
request.interceptors.response.use(
  ...
  async (error: AxiosError<ApiResponse>) => {
    ...
    if (status === 401 || data?.code === ResultEnum.TOKEN_EXPIRED) {
      ...
      // 业务登录模式：非 /cpx/ 接口（框架公告/聊天等）401 不触发刷新/登出，仅静默拒绝。
      // 业务会话对框架接口无权限（无 sys_user 会话），强制登出会误伤正常业务用户。
      if (isBizMode() && !config.url?.includes("/cpx/")) {
        return Promise.reject(new HttpError(data?.msg || $t("httpMsg.unauthorized"), ApiStatus.unauthorized));
      }

      // 已刷新重试过一次仍 401：会话确实失效，不再循环刷新（否则 refresh 200 + 仍 401 会死循环），直接登出
      const retriedConfig = config as InternalAxiosRequestConfig & { _retried?: boolean };
      if (retriedConfig._retried) {
        await redirectToLogin(data?.msg || $t("httpMsg.unauthorized"));
        return Promise.reject(new HttpError(data?.msg || $t("httpMsg.unauthorized"), ApiStatus.unauthorized));
      }
      ...
      if (!isRefreshing) {
        ...
        try {
          ...
          // 用新 token 重试当前请求（标记已重试，避免再次 401 时循环刷新）
