        ...
        <div class="fa-card-sm relative p-9 pb-6 overflow-hidden text-center">
          ...
          <div class="relative z-10 w-75 mx-auto mt-7.5 text-left">
            <div class="mt-2.5 flex items-start">
              <FaSvgIcon icon="ri:gender-line" class="text-g-700 shrink-0 mt-0.5" />
              <span class="ml-2 text-sm">{{ genderText || "—" }}</span>
            </div>
            <div class="mt-2.5 flex items-start">
            ...
            <div class="mt-2.5 flex items-start">
              <FaSvgIcon icon="ri:phone-line" class="text-g-700 shrink-0 mt-0.5" />
              <span class="ml-2 text-sm">{{ infoFormState.mobile || "—" }}</span>
            </div>
          </div>
        </div>
      </div>
...
const genderText = computed(() => {
  const list = dictDataStore.value["sys_user_sex"];
  const g = infoFormState.gender;
  if (!Array.isArray(list)) return "";
  const hit = list.find((item) => String(item.dict_value) === String(g));
  return hit ? hit.dict_label : "";
});
